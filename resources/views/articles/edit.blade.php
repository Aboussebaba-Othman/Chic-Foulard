                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $article->title) }}" placeholder="Entrez le titre de votre article" required>
                                @error('title')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>                            <div class="mb-4">
                                <label for="content" class="form-label fw-bold">Contenu <span class="text-danger">*</span></label>
                                <textarea name="content" id="content" rows="12" class="form-control rich-text @error('content') is-invalid @enderror" placeholder="Rédigez le contenu de votre article ici..." required>{{ old('content', $article->content) }}</textarea>
                                @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="border rounded p-2 text-center">
                                    <img src="{{ asset($article->image) }}" alt="{{ $article->title }}" style="max-height: 200px; max-width: 100%;">
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('articles.show', $article->id) }}" class="btn btn-outline-secondary me-2">Annuler</a>
